...
type ZabbixItems map[string]interface{}
 ...
 */
 func (api *API) GetAllHost() ( []ZabbixHost , error ) {
	 data := make(map[string]interface{}, 0)
	 filter := make(map[string]string, 0)
	 filter["output"] = "hostid,host"
	 filter["selectInterfaces"] = "interfaceid,ip"
	 data["params"] = filter

	 host, err := api.Host("get",data)
...
func GetHost(api *API, host string) (ZabbixHost, error) {
...
/**
get host history till last 5 minite
 */

 func (api *API)GetHostHistotry( host string) ([]ZabbixHistoryItem, error) {
	 params := make(map[string]interface{}, 0)
	 filter := make(map[string]string, 0)
	 filter["host"] = host
	 ...
	 items, err := api.History("get", params)
	 if err != nil {
		 return nil, err
	 }
	 // If our call was successful
 ...
 get host interface
  */
  func (api *API)GetHostItems(hostid string) ( []ZabbixItems,error) {
	  params := make(map[string]interface{}, 0)
	  filter := make(map[string]string, 0)
	  filter["output"] = "extend"
